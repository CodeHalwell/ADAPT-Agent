...
## 2024-06-25 - Prevent Log Poisoning in Firewall Metadata
**Vulnerability:** The firewall recorded truncated content snippets in its security events metadata without escaping newline (`\n`) and carriage return (`\r`) characters. This could allow a malicious actor to inject fake log entries or manipulate log formatting by crafting inputs with these characters.
**Learning:** Even when inputs are truncated to prevent DoS attacks or limit size, they must still be properly sanitized before being recorded in logs or events. Log injection is a distinct risk from DoS.
**Prevention:** Explicitly escape newlines (`\n` to `\\n`) and carriage returns (`\r` to `\\r`) on untrusted content snippets immediately before adding them to log/event metadata.
