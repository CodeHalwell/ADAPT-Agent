...
## 2024-06-23 - Prevent Log Poisoning Attacks in Observability Component
**Vulnerability:** Unsanitized user inputs in `AgentObserver.log_event` and `AgentObserver.log` could be used to inject newlines and forge log entries (log poisoning/injection).
**Learning:** Limiting the input length (e.g., `description[:10000]`) is good for DoS protection, but insufficient for log poisoning. A malicious payload with a newline can still be within the limit and alter log structures.
**Prevention:** Always explicitly escape newline (`\n`) and carriage return (`\r`) characters from untrusted content being logged, after any length truncation operations.
