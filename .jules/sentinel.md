...
## 2026-06-27 - Firewall Log Poisoning and Secret Leakage
**Vulnerability:** The firewall recorded blocked input snippets into security event metadata without explicitly truncating them sufficiently or escaping newline/carriage return characters, creating a log poisoning and potential secret leakage vulnerability.
**Learning:** Even internal security metadata can be a vector for log injection if raw user input strings are formatted unescaped, and failing to cap string lengths closely before logging risks leaking more context than necessary.
**Prevention:** Always explicitly sanitize blocked input snippets and escape newline/carriage return characters (e.g., `.replace('\n', '\\n').replace('\r', '\\r')`) before recording them in `_security_events` metadata to prevent log poisoning.
