...
## 2024-07-01 - Prevent log poisoning in firewall security events
**Vulnerability:** Blocked content snippets stored in security events (via `_redact_snippet`) did not escape newlines and carriage returns.
**Learning:** Even heavily redacted or sanitized output (like the 12-character snippet in `_redact_snippet`) can be manipulated to break log formatting and inject fake entries if control characters like newlines are not escaped.
**Prevention:** Explicitly sanitize and escape newline/carriage return characters before returning snippets or recording them in metadata.
