...
**Vulnerability:** The `AgentObserver` class truncated event `description` and log `message` fields, but it didn't sanitize them to escape newline (`\n`) and carriage return (`\r`) characters. An attacker could exploit this by injecting forged entries via a multiline string in these fields to obscure logs or mislead administrators.
**Learning:** Preventing resource exhaustion vulnerabilities with string truncation doesn't eliminate all security risks from unvalidated input. Untrusted strings logged internally must also be sanitized to prevent log injection or poisoning.
**Prevention:** Apply sanitization (such as escaping `\n` to `\\n` and `\r` to `\\r`) immediately after truncation, before strings are appended to log or event structures.
