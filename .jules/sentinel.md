...
## 2024-06-28 - Log Poisoning and Secret Leakage in Firewall Event Metadata
**Vulnerability:** The firewall's `_check` method recorded the matched snippet (`content[:256]`) in the `content_snippet` metadata without sanitizing newline/carriage return characters. Since it processes untrusted user input, this allowed an attacker to inject `\n` or `\r` and forge log entries (log poisoning). Additionally, a 256-character snippet could leak sensitive secrets in the logs.
**Learning:** Even when inputs are constrained (like `content[:256]`), raw content logged or stored in metadata fields can lead to log injection vulnerabilities and secret leakage if it contains special characters like newlines or if it's overly long.
**Prevention:** Always explicitly escape or sanitize control characters (`\n`, `\r`) and strictly truncate log snippets (e.g., `[:100]`) before persisting them into security events or logs.
