...
## 2024-06-16 - Enum Identity Comparison Optimization
**Learning:** In Python, evaluating `==` between Enums relies on the `__eq__` method, which is generally slower than an identity check `is` (approx. 20% overhead). Since Enum members are singletons, `is` performs identical logic faster.
**Action:** Always prefer identity checks (`is`) over equality checks (`==`) when comparing Python Enums in performance-critical tight loops to bypass `__eq__` method overhead.
