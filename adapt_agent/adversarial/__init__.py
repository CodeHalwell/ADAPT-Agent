...
class AdversarialDefense:
    ...
    def __init__(self, max_attacks: int = 1000, max_content_length: Optional[int] = None):
        ...
        # ⚡ Bolt: Parallel tuple list for O(1) lowercased string matching
    ...
    def detect_custom_pattern(self, prompt: str, prompt_lower: Optional[str] = None) -> bool:
        ...
        """
        prompt_lower = prompt_lower if prompt_lower is not None else prompt.lower()
        # ⚡ Bolt: Iterate over pre-computed tuples to avoid O(N) lower() calls inside the hot loop
        for pattern_orig, pattern_lower in self._attack_patterns_tuple:
            if pattern_lower in prompt_lower:
                self._record_attack("custom_pattern", prompt, pattern_orig)
                return True
