...
class MemorySystem:
    ...
    def store_long_term(
        ...
        if len(self._long_term_memory) > self.long_term_capacity:
            ...
            # Performance optimization: manual for loop is faster than min() or sort()
            # for finding minimum in a list of dicts by avoiding function call overhead
            min_idx = 0
            min_val = self._long_term_memory[0]["access_count"]
            for i in range(1, len(self._long_term_memory)):
                if self._long_term_memory[i]["access_count"] < min_val:
                    min_val = self._long_term_memory[i]["access_count"]
                    min_idx = i
