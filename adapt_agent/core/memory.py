...
class MemorySystem:
    ...
    def store_long_term(
        ...
        if len(self._long_term_memory) > self.long_term_capacity:
            ...
            # O(n) manual loop is faster than O(n log n) sorting for eviction
            min_idx = 0
            min_val = float("inf")
            for i, item in enumerate(self._long_term_memory):
                if item["access_count"] < min_val:
                    min_val = item["access_count"]
                    min_idx = i
