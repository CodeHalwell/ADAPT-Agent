...
class TaintTracker:
    ...
    def get_taint_level(self, data_id: str) -> TaintLevel:
        ...
        highest_level = TaintLevel.UNTAINTED
        highest_priority = 0

        # Optimize performance by avoiding list allocations, .index(), and max()
        # Early return when CRITICAL is found as it's the maximum possible priority
        for sid in source_ids:
            if sid in self._taint_sources:
                level = self._taint_sources[sid].level
                priority = self._LEVEL_PRIORITY[level]
                if priority > highest_priority:
                    highest_priority = priority
                    highest_level = level
                    if highest_priority == 4:  # CRITICAL
                        break
