...
class AgentOptimizer:
    ...
    def _store_suggestions(self, agent_id: str, suggestions: list[dict[str, Any]]) -> None:
        ...
        stored_suggestions = list(suggestions)
        self._suggestions_by_agent[agent_id] = stored_suggestions
        self._suggestion_order.append(agent_id)
        # ⚡ Bolt: Track total suggestions counter for O(1) eviction
        self._total_suggestions += len(stored_suggestions)
        ...
        while (
            ...
            removed = self._suggestions_by_agent.pop(oldest, None)
            if removed is not None:
                self._total_suggestions -= len(removed)
